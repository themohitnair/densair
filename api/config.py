...
- **Key Terms (`key_terms`)**: This is a glossary of all essential technical terms and concepts needed to understand the paper. This must include foundational concepts relevant to the paper, even if they are not explicitly mentioned. Format all terms in bold within the summaries, but not in the key terms list itself. Crucially, identify every technical or domain-specific term used in your explanations and add it to this list. Assume the reader doesn't know even basic terms, even if they are relevant to the paper.
...
- **Markdown Formatting:** Use Markdown for structure, especially bold formatting for terms listed in `key_terms`.
